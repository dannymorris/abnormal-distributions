...
```{r}
credit <- read_csv("_data/_creditcard.csv") %>%
  select(-Time, -Amount) %>%
  mutate(Class = factor(Class, labels = c("ok", "fraud")))

# bcancer <- url("https://archive.ics.uci.edu/ml/machine-learning-databases/breast-cancer/breast-cancer.data") %>%
#   readr::read_csv(., col_names = F) %>%
#   select(-X1) %>%
#   mutate(X11 = ifelse(X11 == "2", "0", "1")) %>%
