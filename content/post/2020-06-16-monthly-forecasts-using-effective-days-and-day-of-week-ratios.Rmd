---
title: Applying calendar adjustments to improve monthly health insurance claims forecasts
author: Danny Morris
date: "`r Sys.Date()`"
output: 
  blogdown::html_page:
...
2. For each month, assign a relative weight to each weekday based on the typical claims volume observed for each weekday
...
## Apply calendar and population adjustments to monthly claims
...
## Forecasting with adjusted PMPM

To test the effectiveness of a monthly claims forecast model built using adjusted PMPM, we compare it to a model built using unadjusted PMPM. Both models are trained using a dedicated training set and evaluated using a dedicated holdout set.
...
In the context of health insurance claims, monthly forecasts can be improved by making calendar and population adjustments to claim counts. The calendar adjustment technique shown in this post involves quantifying the month composition by assessing the distribution of weekdays in a given month and the relative value of a weekday in the same month. This adjustment, along with a population, has been shown to improve the accuracy of genuine forecats.